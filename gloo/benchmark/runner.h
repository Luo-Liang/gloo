...
class RunnerJob {
 public:
  explicit RunnerJob(std::function<void()> fn, std::function<void()> sync, std::function<void()> verify, int i) :
  done_(false), fn_(fn), iterations_(i), sync_(sync), verify_(verify) {}
